...
SAMPLE_FILE_PATH = Path(os.path.join(os.path.dirname(__file__), '..', 'samples', 'power_bi', 'COVID-19 US Tracking Sample.pbix'))
...
def test_measure_extraction(parsed_power_bi_data):
    ...
    total_deaths_measure = next((m for m in measures if m['name'] == 'Total deaths'), None)
