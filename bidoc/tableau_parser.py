...
class TableauParser(MetadataExtractor):
    ...
    def __init__(self):
        super().__init__()
        if Workbook is None:
            raise ImportError("tableau-document-api library is required. Install with: pip install tableau-document-api")
    ...
    def _extract_dashboards(self, workbook) -> List[Dict[str, Any]]:
        ...
        try:
            for dashboard in workbook.dashboards:
                dashboard_info = {
                    'name': dashboard.name,
                    'worksheets': [ws.name for ws in dashboard.worksheets] if hasattr(dashboard, 'worksheets') else [],
    ...
    def _extract_calculated_fields(self, workbook) -> List[Dict[str, Any]]:
        ...
        self.log_extraction_progress("Calculated fields extracted", len(calculated_fields))
