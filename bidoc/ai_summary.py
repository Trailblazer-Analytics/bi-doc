...
class AISummary:
    ...
    def _generate_powerbi_summary(self, metadata: Dict[str, Any], file_name: str) -> str:
        ...
        summary_parts = [
            f"**{file_name}** is a Power BI report containing:"
        ]
        
        # Data model summary
        if tables:
            total_columns = sum(len(table.get('columns', [])) for table in tables)
            summary_parts.append(f"- **{len(tables)} data tables** with {total_columns} total columns")
        
        if measures:
            summary_parts.append(f"- **{len(measures)} DAX measures** for calculations and KPIs")
        
        if relationships:
            summary_parts.append(f"- **{len(relationships)} table relationships** defining the data model")
        
        # Report structure
        if visualizations:
            total_visuals = sum(len(page.get('visuals', [])) for page in visualizations)
            summary_parts.append(f"- **{len(visualizations)} report pages** with {total_visuals} total visualizations")
        
        # Key insights
        summary_parts.append("\n**Key Insights:**")
        
        if measures:
            calc_measures = [m for m in measures if 'SUM(' in m.get('expression', '') or 'CALCULATE(' in m.get('expression', '')]
            if calc_measures:
                summary_parts.append(f"- Contains {len(calc_measures)} advanced calculations using DAX functions")
        
        if relationships:
            summary_parts.append(f"- Data model uses {len(relationships)} relationships to connect tables")
    ...
    def _generate_tableau_summary(self, metadata: Dict[str, Any], file_name: str) -> str:
        ...
        summary_parts = [
            f"**{file_name}** is a Tableau workbook containing:"
        ]
        
        # Data sources
        if data_sources:
            total_fields = sum(len(ds.get('fields', [])) for ds in data_sources)
            summary_parts.append(f"- **{len(data_sources)} data sources** with {total_fields} total fields")
        
        # Worksheets and dashboards
        if worksheets:
            summary_parts.append(f"- **{len(worksheets)} worksheets** for individual visualizations")
        
        if dashboards:
            summary_parts.append(f"- **{len(dashboards)} dashboards** combining multiple worksheets")
        
        if calculated_fields:
            summary_parts.append(f"- **{len(calculated_fields)} calculated fields** for custom metrics")
        
        # Key insights
        summary_parts.append("\n**Key Insights:**")
        
        if calculated_fields:
            summary_parts.append(f"- Uses {len(calculated_fields)} custom calculations for business logic")
        
        if dashboards and worksheets:
            avg_worksheets_per_dashboard = len(worksheets) / len(dashboards) if dashboards else 0
            summary_parts.append(f"- Dashboards average {avg_worksheets_per_dashboard:.1f} worksheets each")
    ...
    def _assess_powerbi_complexity(self, metadata: Dict[str, Any]) -> str:
        ...
        for measure in metadata.get('measures', []):
            ...
            if any(func in expression for func in ['CALCULATE', 'FILTER', 'RELATED', 'USERELATIONSHIP']):
                complex_dax_count += 1
